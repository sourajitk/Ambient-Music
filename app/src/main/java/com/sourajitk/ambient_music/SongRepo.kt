...
object SongRepo {
    ...
    @Volatile
    private var internalLoadedSongs: List<SongAsset> = emptyList()
    ...
    fun initializeAndRefresh(context: Context, onFinished: ((Boolean, String) -> Unit)? = null) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            Log.d(TAG, "initializeAndRefresh: Attempting to fetch songs from remote URL: $REMOTE_SONGS_URL")
            try {
                ...
                client.newCall(request).execute().use { response ->
                    if (response.isSuccessful) {
                        ...
                        if (jsonString != null) {
                            val loggableJson = if (jsonString.length > 500) jsonString.substring(0, 500) + "..." else jsonString
                            Log.d(TAG, "initializeAndRefresh: Successfully fetched JSON string from remote (snippet): $loggableJson")
                            ...
                            synchronized(this@SongRepo) {
                                ...
                                if (currentTrackIndex >= internalLoadedSongs.size && internalLoadedSongs.isNotEmpty()) {
                                    currentTrackIndex = 0
                                }
                                // Save the newly fetched data to cache
                                saveToCache(context, jsonString)
                            }
                            Log.i(TAG, "initializeAndRefresh: Successfully parsed and updated ${remoteSongs.size} songs from remote.")
                        ...
                        Log.e(TAG, "initializeAndRefresh: Remote fetch failed: ${response.code} ${response.message}. Error body: $errorBody")
            ...
            Log.d(TAG, "initializeAndRefresh: Process finished. Songs loaded: ${internalLoadedSongs.size}. Final status: $finalStatusMessage")
