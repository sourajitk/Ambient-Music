...
class MusicPlaybackService : Service() {
    ...
    private fun togglePlayback() {
        ...
        if (playerIsCurrentlyPlaying) {
            ...
            if (currentPlaybackState == Player.STATE_IDLE ||
                currentPlaybackState == Player.STATE_ENDED ||
                exoPlayer?.currentMediaItem?.mediaId != currentSongData.url
            ) {
                Log.d(TAG, "togglePlayback: Preparing MediaItem for URL: ${currentSongData.url} (Title: ${currentSongData.title})")
    ...
    private fun playNextSong(fromUserAction: Boolean) {
        ...
        if (nextSongData != null) {
            ...
            val mediaMetadata = MediaMetadata.Builder()
                .setTitle(nextSongData.title)
                .setArtist(nextSongData.artist)
                .build()
    ...
    private fun createNotification(): Notification {
        Log.d(TAG, "createNotification called. Effective playing state for UI: $isServiceCurrentlyPlaying")
        val exoPlayerMetadata = exoPlayer?.mediaMetadata
        val songFromRepo = SongRepo.getCurrentSong()

        val title = exoPlayerMetadata?.title?.toString()?.takeIf { it.isNotBlank() }
            ?: songFromRepo?.title?.takeIf { it.isNotBlank() }
            //?: getString(R.string.qs_tile_notification_title_unknown)

        val artist = exoPlayerMetadata?.artist?.toString()?.takeIf { it.isNotBlank() }
        ...
        mediaSession?.let { session ->
            ...
            val playPauseIcon = if (playerIsEffectivelyPlaying)
                R.drawable.ic_pause else R.drawable.ic_play_arrow
            val playPauseTitle = if (playerIsEffectivelyPlaying)
                getString(R.string.qs_tile_label_pause) else getString(R.string.qs_tile_label_play)
            val playPauseActionIntent = Intent(this, MusicPlaybackService::class.java).apply {
                action = ACTION_TOGGLE_PLAYBACK_QS
            }
            val playPausePendingIntent = PendingIntent.getService(
                this, 101, playPauseActionIntent,
                PendingIntent.FLAG_IMMUTABLE or PendingIntent.FLAG_UPDATE_CURRENT
            )
            builder.addAction(NotificationCompat.Action(playPauseIcon, playPauseTitle, playPausePendingIntent))

            // Skip to Next Action (index 1 for compact view)
            val skipToNextIntent = Intent(this, MusicPlaybackService::class.java).apply {
                action = ACTION_SKIP_TO_NEXT
            }
            val skipToNextPendingIntent = PendingIntent.getService(
                this, 103, // Ensure unique request code
                skipToNextIntent,
                PendingIntent.FLAG_IMMUTABLE or PendingIntent.FLAG_UPDATE_CURRENT
            )

            val mainActivityIntent = Intent(this, MainActivity::class.java) // Or your MinimalEntryActivity
            val contentPendingIntent = PendingIntent.getActivity(
                this, 102, mainActivityIntent,
                PendingIntent.FLAG_IMMUTABLE or PendingIntent.FLAG_UPDATE_CURRENT
            )
    ...
    private fun requestTileUpdate() {
        Log.d(TAG, "Requesting QS Tile update. Current service playing state: $isServiceCurrentlyPlaying")
